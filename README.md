...
## OmniRoute كبوابة اختيارية

يمكن إضافة **OmniRoute Gateway** كمزوّد OpenAI-compatible مستقل للوصول إلى كتالوج النماذج والمسارات والـcombos التي أعددتها داخل OmniRoute، دون تضمين OmniRoute داخل عملية Node الخاصة بالمشروع أو تغيير إصدار Node في Railway.

- محليًا: شغّل OmniRoute، أنشئ مفتاحًا من صفحة **Endpoints**، ثم استخدم `http://127.0.0.1:20128/v1`.
- على Railway: شغّله كخدمة منفصلة واستخدم رابط HTTPS العام المنتهي بـ`/v1`.
- يمكن اختيار `auto` أو `auto/coding` أو Model ID حقيقي من `/models`.
- لا يمنح التكامل صلاحيات غير موجودة؛ يجب ربط حسابات ومزودات مسموح بها داخل OmniRoute.

راجع [دليل تشغيل OmniRoute مع Moataz AI](docs/OMNIROUTE_PROVIDER.md).
